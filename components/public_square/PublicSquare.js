...
function PublicSquare() {
  ...
  const getWords = async () => {
    try {
      ...
      const { data } = await axios.get(
        ...
        `https://api.datamuse.com/words?rel_trg=food`,
      )
      if (data) {
        //FILTERMAP ARRAY FOR WORDS
        const dataWords = data.map((item) => item.word)
        setTodaysWords(dataWords)
  ...
  const isWordInPoem = (word) => {
    const poemWords = poem.toLowerCase().split(/[\s,]+/)
  ...
  const filterPoem = async () => {
    ...
    if (finalGeneratedWords.every((word) => poemWords.includes(word.toLowerCase()))) {
